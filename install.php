...
/**
 * Write DB credentials to .env (never embed secrets in PHP source).
 */
function install_write_dotenv(string $root, string $host, string $dbname, string $user, string $pass, string $charset = 'utf8mb4'): void
{
    ...
    $lines = [
        '# Bashiqa School Manager — generated by install.php (do not commit)',
        'DB_HOST=' . $escape($host),
        'DB_NAME=' . $escape($dbname),
        'DB_USER=' . $escape($user),
        'DB_PASS=' . $escape($pass),
        'DB_CHARSET=' . $escape($charset),
        '',
    ];
...
/**
 * Ensure default admin exists (development default password; change after login).
 */
function install_ensure_default_admin(PDO $pdo): void
{
    ...
    $ins = $pdo->prepare('INSERT INTO users (username, password_hash, full_name, role, status) VALUES (?, ?, ?, ?, ?)');
    $ins->execute(['admin', $hash, 'System Administrator', 'admin', 'active']);
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'install') {
        ...
        try {
            ...
            // 3. تنفيذ ملف التثبيت (هيكل فقط — database/structure_only.sql)
            $sqlFile = __DIR__ . '/database/structure_only.sql';
            if (!file_exists($sqlFile)) {
                throw new Exception('ملف التثبيت غير موجود: database/structure_only.sql');
            }
            ...
            install_ensure_default_admin($pdo);
            
            // 4. حفظ بيانات الاتصال في .env (لا تُخزَّن في ملفات PHP)
            install_write_dotenv(__DIR__, $host, $dbname, $username, $password, 'utf8mb4');
